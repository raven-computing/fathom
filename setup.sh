#!/bin/bash
# Setup script for the Fathom project.
...
function __install_py_dependencies() {
  local requirements_file="requirements.txt";
  if [ -r "$requirements_file" ]; then
    logI "Installing Python dependencies";
    if ! ${_PIP_EXEC} install -r "${requirements_file}"; then
...
function setup_virtual_env() {
  ...
  local virtualenvwrapper_script="";
  ...
  if ! __install_py_dependencies; then
    return 1;
  fi
